...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T)
    untitleddddddddddddddd_M->Timing.clockTick0;
  untitleddddddddddddddd_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(untitleddddddddddddddd_M, 0);
  ...
  untitleddddddddddddddd_initialize();
  cli();
  sei ();

  /* External Mode initialization */
  errorCode = extmodeInit(untitleddddddddddddddd_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(untitleddddddddddddddd_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(untitleddddddddddddddd_M);
  ...
  /* Terminate model */
  untitleddddddddddddddd_terminate();
