...
void rt_OneStep(void)
{
   ...
   */
  trying_SetEventsForThisBaseStep(eventFlags);
  ...
  currentTime = (extmodeSimulationTime_T) ((trying_M->Timing.clockTick0 * 1) + 0)
    ;
  trying_step0();
  ...
  if (need2runFlags[1]) {
    ...
    {
     case 1 :
      currentTime = (extmodeSimulationTime_T) ((trying_M->Timing.clockTick1 *
        100) + 0)
        ;
      trying_step1();
...
int main(void)
{
  ...
  errorCode = extmodeInit(trying_M->extModeInfo, &rtmGetTFinal(trying_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(trying_M, true);
    }
  }
